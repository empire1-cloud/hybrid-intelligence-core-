...
ENGINE_MAP is declared and never read. These tests prove the new, additive
`CanonicalOrchestrator` actually dispatches an Analysis-classified prompt to
AnalysisEngine, that a simulated primary-model failure produces a real
fallback (not a silent single point of failure), and that a fabricated
parse-failure response is refused rather than persisted as a canon output.
...
from services.hybrid_core import TaskType  # noqa: E402
...
def test_analysis_task_actually_reaches_analysis_engine(monkeypatch):
    ...
    result = asyncio.run(
        orchestrator.execute(prompt="Analyze our retention curve", task_type=TaskType.ANALYSIS)
    )
...
def test_plan_task_reaches_plan_builder_engine(monkeypatch):
    ...
    orchestrator = CanonicalOrchestrator()
    result = asyncio.run(orchestrator.execute(prompt="Build a rollout plan", task_type=TaskType.PLAN))
...
def test_general_task_falls_back_to_strategy_engine(monkeypatch):
    ...
    orchestrator = CanonicalOrchestrator()
    result = asyncio.run(orchestrator.execute(prompt="What is our best move?", task_type=TaskType.GENERAL))
...
def test_orchestrator_falls_back_when_primary_model_fails(monkeypatch):
    ...
    orchestrator = CanonicalOrchestrator()
    result = asyncio.run(
        orchestrator.execute(prompt="What is our best move?", task_type=TaskType.STRATEGY, force_model="gpt-5.2")
    )
...
def test_orchestrator_refuses_to_canonize_a_fabricated_fallback(monkeypatch):
    """StrategyEngine's own JSONDecodeError fallback must not be presented as a real canon output."""
    ...
    orchestrator = CanonicalOrchestrator()
    result = asyncio.run(orchestrator.execute(prompt="What is our best move?", task_type=TaskType.STRATEGY))
...
def test_orchestrator_never_dispatches_a_blocked_model(monkeypatch):
    ...
    orchestrator = CanonicalOrchestrator()
    result = asyncio.run(
        orchestrator.execute(prompt="x", task_type=TaskType.GENERAL, force_model="gemini-3-flash")
    )
