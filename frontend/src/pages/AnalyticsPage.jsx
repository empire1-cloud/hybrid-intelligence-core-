...
const AnalyticsPage = () => {
  ...
  const checkAlerts = useCallback((health, drift) => {
    ...
    newAlerts.forEach((alert) => {
      if (alert.type === "critical") {
        toast.error(
          `🚨 ${alert.metric} Alert: ${alert.value ? `${alert.value.toFixed(1)}%` : alert.message}`,
          {
            duration: 5000,
          },
        );
  ...
  const updateSparklines = useCallback((usage, latency) => {
    setSparklineData((prev) => {
      ...
      usage.forEach((u) => {
        ...
        newData[u.engine].usage = [
          ...(newData[u.engine].usage || []).slice(-11),
          { t: timestamp, v: u.count },
        ];
      });

      latency.forEach((l) => {
        if (!newData[l.engine]) newData[l.engine] = { usage: [], latency: [] };
        newData[l.engine].latency = [
          ...(newData[l.engine].latency || []).slice(-11),
          { t: timestamp, v: l.avg_latency_ms },
        ];
      });
  ...
  const GaugeCard = ({
    ...
    return (
      <div
        ...
        data-testid={`gauge-${label.toLowerCase().replace(" ", "-")}`}
      >
        ...
        <div className="gauge-ring">
          ...
          <div className="gauge-value">
            <span
              className={`gauge-number ${isIncreasing ? "flash-up" : ""} ${isDecreasing ? "flash-down" : ""}`}
            >
              {value?.toFixed(1)}
            </span>
  ...
  if (loading) {
    return (
      <div
        className={`page-container analytics-page ${theme}`}
      ...
      <header className="page-header">
        ...
        <div className="header-content">
          ...
          <p className="subtitle">
            Real-time system performance and AI quality metrics
          </p>
        </div>
        ...
        <div className="stats-row quick-stats" data-testid="quick-stats">
          <div className="stat-card animated-card">
            ...
            <div className="stat-content">
              ...
              <span className="stat-value animate-value">
                {realtimeStats?.total_executions || 0}
              </span>
            </div>
          </div>
          <div className="stat-card animated-card">
          ...
          <div className="stat-card animated-card">
            ...
            <div className="stat-content">
              ...
              <span className="stat-value animate-value">
                {(realtimeStats?.avg_duration_ms / 1000)?.toFixed(1) || 0}s
              </span>
            </div>
          </div>
          <div className="stat-card animated-card">
            ...
            <div className="stat-content">
              ...
              <span className="stat-value animate-value">
                {realtimeStats?.recent_5min?.count || 0} calls
              </span>
            </div>
          </div>
        </div>
            ...
            {widgets.engineLatency && (
              <div className="chart-card large animated-card">
                <div className="chart-header">
                  ...
                  <span className="chart-subtitle">
                    Latency in milliseconds per engine
                  </span>
                </div>
                <div className="chart-body">
                        ...
                        <tbody>
                          {engineErrors.map((e, i) => (
                            <tr key={i} className="animated-row">
                              ...
                              <td>
                                <span
                                  className={`severity-badge ${e.severity}`}
                                >
                                  {e.severity}
                                </span>
                              </td>
            ...
            {widgets.confidenceTrends && (
              <div className="chart-card large animated-card">
                <div className="chart-header">
                  ...
                  <span className="chart-subtitle">
                    AI confidence over the last 12 hours
                  </span>
                </div>
                <div className="chart-body">
              ...
              <div className="chart-card animated-card">
                ...
                <div className="chart-body">
                  <div className="drift-grid" data-testid="drift-grid">
                    {driftStatus.length > 0 ? (
                      driftStatus.map((d, i) => (
                        <div
                          key={i}
                          className={`drift-card ${d.status} ${d.status === "red" ? "alert-pulse" : ""}`}
                        >
                          <div className="drift-header">
                            <StatusIndicator status={d.status} />
                            <span className="drift-engine">
                              {d.engine
                                .replace(/_/g, " ")
                                .replace("engine", "")}
                            </span>
                          ...
                          <div className="drift-body">
                            <div className="drift-trend">
                              ...
                              <span
                                className={`trend-value ${d.confidence_trend}`}
                              >
            ...
            {widgets.modelComparison && (
              <div className="chart-card animated-card">
                <div className="chart-header">
                  ...
                  <span className="chart-subtitle">
                    Performance metrics across LLM models
                  </span>
                </div>
                <div className="chart-body">
        ...
        {activeTab === "system" && (
          <div className="tab-panel" data-testid="panel-system">
            ...
            <div className="metrics-source-banner" data-testid="metrics-source">
              <span
                className={`source-indicator ${systemHealth?.psutil_available ? "real" : "mock"}`}
              >
                {systemHealth?.psutil_available
                  ? "🟢 Real Metrics (psutil)"
                  : "🟡 Mock Metrics (psutil unavailable)"}
              </span>
                  ...
                  <div className="detailed-stat-body">
                    <div className="stat-row">
                      ...
                      <span className="stat-val">
                        {systemHealth?.memory_total_gb || 0} GB
                      </span>
                    </div>
                    <div className="stat-row">
                      <span className="stat-key">Used:</span>
                      <span className="stat-val">
                        {systemHealth?.memory_used_gb || 0} GB
                      </span>
                    </div>
                    <div className="stat-row">
                ...
                <div className="detailed-stat-card animated-card">
                  ...
                  <div className="detailed-stat-body">
                    <div className="stat-row">
                      <span className="stat-key">Total:</span>
                      <span className="stat-val">
                        {systemHealth?.disk_total_gb || 0} GB
                      </span>
                    </div>
                    <div className="stat-row">
                      <span className="stat-key">Used:</span>
                      <span className="stat-val">
                        {systemHealth?.disk_used_gb || 0} GB
                      </span>
                    </div>
                    <div className="stat-row">
                      <span className="stat-key">Usage:</span>
                    ...
                    {systemHealth?.load_average ? (
                      <>
                        <div className="stat-row">
                          ...
                          <span className="stat-val">
                            {systemHealth.load_average[0]}
                          </span>
                        </div>
                        <div className="stat-row">
                          <span className="stat-key">5 min:</span>
                          <span className="stat-val">
                            {systemHealth.load_average[1]}
                          </span>
                        </div>
                        <div className="stat-row">
                          <span className="stat-key">15 min:</span>
                          <span className="stat-val">
                            {systemHealth.load_average[2]}
                          </span>
                        </div>
                      </>
                    ) : (
              ...
              <div className="system-stats-row">
                <div className="system-stat-card animated-card">
                  ...
                  <div className="system-stat-content">
                    <span className="system-stat-value animate-value">
                ...
                <div className="system-stat-card animated-card">
                  ...
                  <div className="system-stat-content">
                    <span className="system-stat-value animate-value">
                      {systemHealth?.uptime_hours?.toFixed(1) || 0}h
                    </span>
                ...
                <div className="system-stat-card animated-card">
                  ...
                  <div className="system-stat-content">
                    <span
                      className={`system-stat-value ${systemHealth?.status}`}
                    >
            ...
            {widgets.pipelineFlow && (
              <div className="chart-card large animated-card">
                <div className="chart-header">
                  ...
                  <span className="chart-subtitle">
                    Current pipeline flow and throughput
                  </span>
                </div>
                <div className="chart-body">
                  <div className="pipeline-stats">
                    <div className="pipeline-stat">
                      <span className="pipeline-stat-value">
                        {pipelineGraph?.active_pipelines || 0}
                      </span>
                    ...
                    <div className="pipeline-stat">
                      <span className="pipeline-stat-value">
                        {pipelineGraph?.queue_length || 0}
                      </span>
                      <span className="pipeline-stat-label">Queue Length</span>
                    </div>
                    <div className="pipeline-stat">
                      <span className="pipeline-stat-value">
                        {pipelineGraph?.throughput_per_minute || 0}/min
                      </span>
                  ...
                  <div className="pipeline-flow" data-testid="pipeline-flow">
                    {pipelineGraph?.nodes?.map((node, i) => (
                      <div key={node.id} className="pipeline-flow-item">
                        <div
                          className={`flow-node ${node.type} ${node.status}`}
                        >
