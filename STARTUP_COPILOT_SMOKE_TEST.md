...
| No stub responses in the engines | `grep -E "mock|placeholder|TODO|lorem|demo|fallback|dummy|NotImplemented"` over engines and routes → no matches |

Run them:

```bash
cd frontend && CI=true npm run build
cd backend  && PYTHONPATH=. python -m pytest -q tests/test_startup_copilot_contract.py
```
...
- [ ] Output shape matches what `SkillResult` renders (the renderer handles arbitrary nesting, but the actual response shape is unconfirmed)
